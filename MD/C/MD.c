...
/* set the viscosity term in the force calculation */
	  ...
	  //add_norms(Nbody,r,pos[j]);
          wind_forces(Nbody,f[j],vis,wind[j]);			// change 1.1 (loop fusion)
        }
/* add the wind term in the force calculation */
/*        for(j=0;j<Ndim;j++){					// 3 times. change 1.1 (loop fusion)
...
/* update positions */
        for(i=0;i<Nbody;i++){					// 4096
          for(j=0;j<Ndim;j++){					// 3
            ...
            velo[j][i] = velo[j][i] + dt * (f[j][i]/mass[i]);	// 1.0 performance: loop fusion
          }
        }
